...
**処理は端末の中だけで完結します。** 写真はどこにも送信しません。そのために OCR エンジン
（[Tesseract](https://github.com/naptha/tesseract.js)）を `vendor/` に同梱しています。
初回だけエンジンと文字データを読み込みます（**合計 9MB 程度**）。以後はキャッシュされ、
オフラインでも使えます。
...
画面のいちばん下に版（`v8` など）が出ます。更新したのに変わらないときはここを見てください。
...
npm test    # 115 tests（Node 18+ 同梱のテストランナー、追加インストール不要）
```
