...
- 初回だけエンジンと文字データを読み込みます（**合計 9MB 程度**）。以後はキャッシュされ、
  オフラインでも使えます。起動を重くしないよう、事前キャッシュには入れていません。
- 既定は英字のみ。日本語のタグは「日本語も読む」を入れてください（少し遅くなります）。
- 日本語は OCR が「サイ ズ M」のように文字の間を切ってくるので、かな・漢字どうしの
  空白は詰めてから使います。詰めないとサイズもブランドも照合に引っかかりません。
- 読み取りの信頼度が 60% 未満なら「読み取れませんでした」として**何も入れません**。
  布のしわを字と見なした幻の文字（`HI` など）が混ざるのを防ぐためです。
- 結果は編集できるテキスト欄に入ります。**間違いは手で直せます。**

読み取り精度はタグ次第です。印刷がはっきりした品番やブランド名はよく読めますが、
織りネームや小さい取扱表示は苦手です。明るい場所でタグを画面いっぱいに写すと当たります。
...
画面のいちばん下に版（`v7` など）が出ます。更新したのに変わらないときはここを見てください。
...
npm test    # 110 tests（Node 18+ 同梱のテストランナー、追加インストール不要）
```
...
js/
  ...
  ocr.js                写真 → 文字（下処理・信頼度の判定・日本語の空白詰め）
...
- OCR は初回に 9MB 程度の読み込みが要ります。織りネームや小さい取扱表示は読み落とします。
